#!/bin/bash
{
	...
	export LOGNAME=`date "+%Y.%m.%d.d25.log"`
	...
	nohup java -jar ${JAR}  --host=192.168.1.200 --port=30003 --lat=29.88058 --lon=-98.23455 --range=25
	#nohup java -jar ${JAR}  --host=192.168.1.200 --port=30003 --lat=29.88058 --lon=-98.23455 --range=10 >> ${LOGS}/${LOGNAME} &
	#tail -f ${LOGS}/${LOGNAME}
}
